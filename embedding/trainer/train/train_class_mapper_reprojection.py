...
class Trainer:
    ...
    def enable_tensorboard(self, log_dir: str = None, save_image_log=False):
        ...
        print("TensorBoard logging enabled. dir: ",tensorboard_dir)
    ...
    def set_train_dataloader(
        ...
        self.train_dataloader = DataLoader(
            dataset, batch_size=self.cfg["train"]["batch-size"], shuffle=True, num_workers=0
        )
    ...
    def set_validation_dataloader(
        ...
        self.val_dataloader = DataLoader(
            dataset, batch_size=self.cfg["validation"]["batch-size"], shuffle=True, num_workers=0
        )
    ...
    def _train(self, current_epoch):
        ...
        for data, target in tqdm(self.train_dataloader):
            # print('*', end="")
            data, target = data.to(self.device).to(torch.float32), target.to(self.device).to(torch.float32)

            self.optimizer.zero_grad()

            output, reprojection = self.model(data)

            f_loss, f_losses = self._get_loss(output=output, target=target)
            re_loss, re_losses = self._get_loss(output=reprojection, target=data)
            loss = f_loss + re_loss
        ...
        if self.save_ckpt:
            ...
            if len(self.best_ckpt) < 5:
                ...
                if total_val_loss < max(score_list):
                    drop_idx = score_list.index((max(score_list)))
